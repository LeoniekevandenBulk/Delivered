...
def show_norm_slices(vol_slices, seg_slices):
    """ Function to display row of image slices """
    nslice = len(vol_slices)
    fig, axes = plt.subplots(2, nslice)
    for i, slice in enumerate(vol_slices):
        im0 = axes[0, i].imshow(slice.T, cmap="gray", origin="lower")
    for i, slice in enumerate(seg_slices):
        im1 = axes[1, i].imshow(slice.T, cmap="gray", origin="lower")
...
def show_volumes(vol_batch, seg_batch, dir):
    for i, (vol, seg) in enumerate(zip(vol_batch, seg_batch)):
        ...
        show_norm_slices([vol_slice_0, vol_slice_1, vol_slice_2], [seg_slice_0, seg_slice_1, seg_slice_2])
        plt.suptitle("Center slices for 3D volume "+vol, size=40)
        #plt.show(block=False)
        #plt.pause(0.5)
        plt.savefig(dir+'volume_'+str(i)+'.png')

def show_deepMedic_prediction(vol, seg, pred, prob, show_dir):

    vol_proxy = nib.load(vol)
    vol_array = vol_proxy.get_data()
    seg_proxy = nib.load(seg)
    seg_array = seg_proxy.get_data()
    pred_proxy = nib.load(pred)
    pred_array = pred_proxy.get_data()
    prob_proxy = nib.load(prob)
    prob_array = prob_proxy.get_data()
    name=vol.split('-')[1].split('.')[0]
    mid_0 = int(vol_array.shape[0]/2)
    mid_1 = int(vol_array.shape[1]/2)
    mid_2 = int(vol_array.shape[2]/2)
    ...
    seg_slice_1 = seg_array[:, mid_1, :]
    seg_slice_2 = seg_array[:, :, mid_2]
    pred_slice_0 = pred_array[mid_0, :, :]
    pred_slice_1 = pred_array[:, mid_1, :]
    pred_slice_2 = pred_array[:, :, mid_2]
    prob_slice_0 = prob_array[mid_0, :, :]
    prob_slice_1 = prob_array[:, mid_1, :]
    prob_slice_2 = prob_array[:, :, mid_2]
    show_slices_x4([vol_slice_0, vol_slice_1, vol_slice_2],
                     [seg_slice_0, seg_slice_1, seg_slice_2],
                     [pred_slice_0, pred_slice_1, pred_slice_2],
                     [prob_slice_0, prob_slice_1, prob_slice_2])
    plt.suptitle("Center slices for 3D volume "+vol, size=40)
    #plt.show(block=False)
    #plt.pause(0.5)
    plt.savefig(show_dir+'volume_'+name+'.png')
