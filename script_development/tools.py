...
def show_segmentation_prediction(batchGenerator, trainer, vol_list, batch_dir,
    ...
    for i in range(nr_test_batches):
        ...
        loss, l2_loss, accuracy, target_prediction, prediction = trainer.validate_batch(X_val, Y_val,
                                                                                              verbose=True)
        target_preds.append(target_prediction.reshape(404,404))
        val_preds.append(prediction.reshape(404,404,2)[:,:,1])
