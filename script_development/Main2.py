...
if (load_liver_segmentation):
    ...
    liver_network, threshold = trainer.trainNetwork(liver_segmentation_name, 
            patch_size, depth, branching_factor, out_size, image_center,
            train_batch_dir, inputs, targets, weights, 
            "liver", train_data, val_data,
...
if (load_lesion_detection):
    lesion_network = trainer.loadNetwork(lesion_detection_name, patch_size, 
            inputs, targets, weights, depth, branching_factor)
else:
    lesion_network, threshold = trainer.trainNetwork(lesion_detection_name, 
            patch_size, depth, branching_factor, out_size, image_center,
            train_batch_dir, inputs, targets, weights, 
            "lesion", train_data, val_data,
            lesion_aug_params, learning_rate,
            nr_epochs, nr_train_batches, nr_val_batches, batch_size,
            mask_network = liver_network, threshold = threshold)
