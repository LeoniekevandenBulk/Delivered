...
class UNetClass:
	...
	self.nonlinearity = nonlinearities.rectify
	...
	# Layers that go 'down' in the network
	def contraction(depth, deepest):
            n_filters = self._num_filters_for_depth(depth, self.branching_factor)
            ...
            net['conv{}_1'.format(depth)] = Conv2DLayer(incoming, name = 'conv{}_1'.format(depth),
							    num_filters=n_filters, filter_size=3, pad=self.pad,
        ...
        # Layers that go 'up' in the network		
        def expansion(depth, deepest):
            ...
            net['bridge{}'.format(depth)] = ConcatLayer([net['upconv{}'.format(depth)],
	...
	# Output layer
        net['out'] = Conv2DLayer(net['_conv0_2'], num_filters=self.num_classes, filter_size=(1,1), pad=self.pad, nonlinearity=None)

        print ('Network output shape {}'.format(str(net['out'].output_shape)))

        return net['out']

    # Define amount of filters needed in network	
    def _num_filters_for_depth(self,depth, branching_factor):
        return 2**(branching_factor+depth)

    # Define image size after a run through the network	
    ...
    # Define updates and functions to train network	
    ...
    # Metrics to evaluate performance of network, used to update the network				
